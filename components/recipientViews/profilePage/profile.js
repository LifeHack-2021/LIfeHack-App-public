import React, { useState, useEffect } from 'react';
import { Badge, Text, Progress, Image, Center, HStack, Stack, Button, Box, Divider, ScrollView } from 'native-base';
import { RefreshControl } from "react-native";

import axios from 'axios'
import { SERVER_URL } from '../../variables';

import EditProfileForm from './editForm';
import FriendsView from './friendsView';

export default function RecipientProfile(props) {
    let [userInfo, setUserInfo] = useState(props.userInfo)
    let [currentDisplay, setCurrentDisplay] = useState("editProfile")

    let [data, setData] = useState([])
    let [loading, setLoading] = useState(true);

    let [friendsNo, setFriendsNo] = useState(0)
    let [friends, setFriends] = useState([])

    let refresh = () => {
        // axios.post(`${SERVER_URL}/items/getRecipients`,
        //     {
        //         username: props.userInfo.username
        //     })
        //     .then(res => {
        //         setData(res.data)
        //         setLoading(false)
        //     })
        //     .catch(err => { console.log(err) })

        axios.post('http://13.229.188.217/items/getRecipientItems', {
            'username': props.userInfo.username
        })
            .then(res => {
                let data = res.data
                let friends = {}
                data.forEach(e => {
                    if (e.donor != "none") {
                        friends[e.donor] = 0
                    }
                })
                setFriends(Object.keys(friends))
                setFriendsNo(Object.keys(friends).length)
            })
            .catch(err => {
                console.log(err)
            })

        axios.get(`http://13.229.188.217/users/getUserInfo/${props.userInfo.username}`, {
        })
            .then(res => {
                let data = res.data
                setUserInfo(res.data)
            })
            .catch(err => {
                console.log(err)
            })

    }

    const reviewList = [];
    const activeList = [];
    data.forEach(element => {
        if (element.status == 'received') {
            reviewList.push(element);
        }
    })

    data.forEach(element => {
        if (element.status != 'received') {
            activeList.push(element);
        }
    })

    useEffect(() => {
        if (loading == true) {
            refresh()
            setLoading(false)

        }

    })

    return (
        <>
            <Box
                bg={{
                    linearGradient: {
                        colors: ["lightBlue.300", "violet.800"],
                        start: [0, 0],
                        end: [1, 0],
                    },
                }}
                m={2}
                p={6}
                rounded="lg"
                _text={{
                    fontSize: "md",
                    fontWeight: "bold",
                    color: "white",
                }}
            >
                <Center my='auto'>
                    <Image
                        source={{ uri: 'data:image/jpeg;base64,' }}
                        size={100}
                        alt='profile picture'
                        style={{
                            borderRadius: 100,
                        }}
                    />

                    <Text fontSize={30} color="white" marginTop={3}>{userInfo.name}</Text>
                    {/* {console.log(userInfo.role)} */}
                    <Text fontSize={15} color="white" marginTop={3}>
                        {/* {console.log(userInfo.role)} */}
                        Recipient
                    </Text>

                </Center>
            </Box>

            {/* Button Controls to Switch Views */}
            <Stack py={1} space={3} alignItems="center">
                <HStack space={3} alignItems="center">
                    <Button
                        colorScheme="secondary"
                        variant="outline"
                        onPress={() => { setCurrentDisplay("editProfile") }}
                        mx={{
                            base: "auto",
                            md: 0,
                        }}
                    >
                        Edit profile
                    </Button>
                    <Button
                        variant="outline"
                        onPress={() => { setCurrentDisplay("Friends") }}
                        endIcon={
                            <Badge colorScheme="primary" ml={1} borderRadius={100}>
                                <Text>{friendsNo}</Text>
                            </Badge>
                        }
                        mx={{
                            base: "auto",
                            md: 0,
                        }}
                    >
                        Friends
                    </Button>
                </HStack>
            </Stack>

            {/* Views */}
            {
                <ScrollView
                    refreshControl={
                        <RefreshControl
                            refreshing={false}
                            onRefresh={refresh}
                        />
                    }
                >
                    {(currentDisplay == "editProfile") ? <EditProfileForm userInfo={userInfo} />
                        : <FriendsView toChat={props.toChat} userInfo={userInfo} setUserInfo={setUserInfo} setFriendsNo={setFriendsNo} friends={friends} />}
                    {/* // <FriendsView userInfo={props.userInfo} setFriendsNo={setFriendsNo} friends={friends} />} */}
                </ScrollView>
            }
        </>
    )
}

